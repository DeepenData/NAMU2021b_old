...
list(Hyperhubs,
Optimal_hubs,
Central_hubs) %>% map(get_node_and_module) %>% reduce(rbind) %>% set_colnames(c("Cell","Module")) -> Cell_Module
Cell_Module %>% count(Cell) %>% filter(Cell == "Astrocyte") %>% .[[2]] -> astro_total
Cell_Module %>% count(Cell) %>% filter(Cell == "Neuron")    %>% .[[2]] -> neuron_total

Cell_Module %>% count(Cell,Module) %>% filter(Cell == "Astrocyte") %>% .[["n"]] -> modules_astro
Cell_Module %>% count(Cell,Module) %>% filter(Cell == "Neuron") %>% .[["n"]]    -> modules_neuron
Cell_Module %>% count(Cell,Module) %>% mutate("Percentage %" =  100* c(modules_astro/astro_total,modules_neuron/neuron_total) %>% round(4)  ) -> my_counts
