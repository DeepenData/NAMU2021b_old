...
#cut_off_optimality <-.5
#cut_off_centrality <- 1.10  

cut_off_optimality <-.425
cut_off_centrality <- 0.5 

 optimalhubs.xlsx <- read_excel('optimalhubs.xlsx') %>% filter(total > cut_off_optimality) %>% filter_out_subsystems
 centralhubs.xlsx <- read_excel('centralhubs.xlsx') %>% filter(total.abs.centrality > cut_off_centrality)%>% filter_out_subsystems
 hyperhubs.xlsx   <- read_excel('hyperhubs.xlsx')%>% filter(total > cut_off_optimality | total.abs.centrality > cut_off_centrality)%>% filter_out_subsystems
```
...
compareCluster(hubs, fun='enrichKEGG', pvalueCutoff=5e-2) -> clusters_plot
clusters_plot %>% dotplot
```
